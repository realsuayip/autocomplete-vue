<template>
  <div style="max-width: 500px; margin: 0 auto">
    <h4 style="margin-bottom: .5rem;">Select a country: </h4>
    <AutoComplete @selected="onSelect" @lookup="callAPI" :highlight="true" no-results-message="No corresponding results." :cache="true"/>
    <br>
    <span>Latest selected country: {{ country.name }}</span>
  </div>
</template>
  ...
  methods: {
    ...
    async callAPI (query, done) {
      ...
      const countryList = ["Afghanistan", "Albania", "Algeria", "Andorra", "Angola", "Anguilla",
        ...
        "St. Lucia", "Sudan", "Suriname", "Swaziland", "Sweden", "Switzerland", "Syria", "Taiwan", "Tajikistan", "Tanzania", "Thailand", "Timor L'Este",
        "Togo", "Tonga", "Trinidad & Tobago", "Tunisia", "Turkey", "Turkmenistan", "Turks & Caicos", "Uganda", "Ukraine", "United Arab Emirates",
        "United Kingdom", "Uruguay", "Uzbekistan", "Venezuela", "Vietnam", "Virgin Islands (US)", "Yemen", "Zambia", "Zimbabwe"]

      let results = await new Promise(resolve => {
        const results = countryList.filter(c => c.toLowerCase().startsWith(query.toLowerCase())).map(c => {
